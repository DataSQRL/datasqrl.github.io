...
DataSQRL eliminates most of the laborious code of stitching together data layers.
...
3. **Compile Data Layer:** DataSQRL compiles the SQRL script and API specification into a fully integrated data layer. The compiled data layer ingests the imported data, processes it according to the transformations and analyses defined in the SQRL script, and serves the resulting data through the specified API.

In a nutshell, DataSQRL is an abstraction layer that takes care of the nitty-gritties of building efficient data layers and gives developers an easy-to-use tool to build custom data APIs.
...
Okay, you get the idea of a compiler for data services that produces integrated data layers. But what exactly does DataSQRL do for you? Glad you asked.

To produce fully integrated data layer, the DataSQLR compiler:
* resolves data imports to data source connectors and generates input schemas for the stream ingestion,
* synchronizes data schemas and data management across all engines in the data layer,
* aligns timestamps and watermarks across the engines,
* orchestrates the layers for optimal data flow between engines,
...
To produce high-performance data services that respond to new input data in realtime and provide low latency, high throughput APIs to many concurrent users, DataSQRL optimizes the compiled data layer by:
...
- Learn more about the [DataSQRL Optimizer](/docs/reference/operations/optimizer) and how the DataSQRL compiler generates efficient data layers.

## Footnotes

**[1]** DataSQRL supports arbitrary DAG topologies for the underlying data layer and additional types of data engines. This is a simplification for the most common use cases.