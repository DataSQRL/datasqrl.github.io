...
SQRL uses the term **now** to designate the point in time to which the data layer has caught up in processing stream records. Now is the present moment from the perspective of the data layer. Now is marked as the orange vertical bar on the timeline.

Now is always behind the present. Now monotonically advances like the present, but it may not advance smoothly. If the data layer is operating with low latency, now can be just a few milliseconds behind the present. If stream records arrive with delay or the data layer is under a lot of load, now can be multiple seconds behind the present. And if the data layer crashes and restarts, now may fall minutes or hours behind the present and then catches back up as stream records are processed.
...
:::info
To summarize, use `now()` for recency comparisons and to refer to the present time in the data layer. 
:::
...
The timestamp of a stream table determines how stream records are associated with a point on the timeline and how now advances in the data layer.
...
The DataSQRL compiler synchronizes time between the components and systems of the data layer to ensure that all systems agree on now. However, millisecond imprecisions can arise at system boundaries due to communication overhead. 
