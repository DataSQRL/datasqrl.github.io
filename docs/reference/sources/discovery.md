...
Data discovery can analyze the data of source tables to produce statistical information that the [DataSQRL optimizer](../../operations/optimizer) uses to produce more efficient data layers.
...
Data analysis runs a full data layer as part of data discovery. The data layer is generated by DataSQRL and executed on the local machine where the command is executed. The data layer can be configured through the [DataSQRL configuration](../../operations/package-config).
